...
export default function TenderLayout({ children }: { children: ReactNode }) {
    ...
    useEffect(() => {
        ...
        const loadTender = async () => {
            ...
            // If we already have a tender and it's the right one, don't trigger a full reload/error flow
            // unless we specifically want to refresh. This prevents transient sync issues from triggering redirects.
            if (tender && tender.id === id && !error) {
                const refreshedTender = await getTender(id);
                if (refreshedTender) setTender(refreshedTender);
                return;
            }
            ...
            if (id === 'new') {
                ...
                if (fetchedTender) {
                    ...
                    // Only redirect if we are truly sure the tender doesn't exist
                    // and we haven't already loaded it once.
                    if (!tender && !isTendersLoading) {
        ...
        loadTender();
    }, [id, getTender, isTendersLoading, isAuthLoading, user, router, stableToast, error, tender]);
