...
import { Search, PlusCircle, Loader2, Download, TrendingUp, TrendingDown, Wallet, FileSpreadsheet } from 'lucide-react';
...
function CollectorsDepositWorksContent() {
  ...
  const { collectorDepositWorkEntries } = useMemo(() => {
    ...
    return { collectorDepositWorkEntries: entries };
  ...
  const handleExportCSV = () => {
    ...
    const headers = ["Sl No", "File No", "Applicant Name", "Application Type", "Sites", "Purposes", "Sanctions Total", "Expenditure Total", "Net Balance", "File Status"];
    ...
    link.setAttribute("download", `GWD_Collectors_Deposit_Works_${activeTab}_${new Date().toISOString().slice(0,10)}.csv`);
