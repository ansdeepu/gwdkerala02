...
const ReportButton = ({
  ...
  const icon = isLoading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Download className="mr-2 h-4 w-4" />;

  const buttonContent = (
      <Button onClick={handleClick} variant="outline" type="button" className="justify-start w-full" disabled={disabled || isLoading}>
          {icon}
          {label}
      </Button>
  );
...
const CorrigendumReportButton = ({ corrigendum, index }: { corrigendum: Corrigendum, index: number }) => {
    ...
    return (
        <Button variant="outline" type="button" className="justify-start w-full" onClick={handleGenerate} disabled={isLoading || !corrigendum.corrigendumType}>
