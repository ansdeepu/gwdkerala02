    ...
    allFileEntries.forEach(entry => {
        ...
        entry.reappropriationDetails?.forEach(reapp => {
            if (reapp.refFileNo?.toLowerCase().trim() === normalizedFileNo) {
                ...
                if (reapp.pageType && !matchPageTypeWithModuleCategory(reapp.pageType, 'logging_pumping_test')) {
                    return;
                }
                const hasInvestigation = entry.siteDetails?.some(s => s.purpose === 'GW Investigation');
                const hasLoggingPumping = entry.siteDetails?.some(s => s.purpose && LOGGING_PUMPING_TEST_PURPOSE_OPTIONS.includes(s.purpose as any));
                  ...
                  <Card className="border-l-4 border-l-amber-500 shadow-xs">
                    <div className="flex items-center justify-between border-b px-6 py-4">
                      <div className="flex-1">
                        <AccordionTrigger className="p-0 hover:no-underline [&[data-state=open]]:border-b-0">
                          <div className="flex items-center gap-3 text-left">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-muted-foreground mt-0.5">Fund transfers, credits, and debits across accounts</p>
                            </div>
                          </div>
                        </AccordionTrigger>
                      </div>
                      <div className="flex items-center gap-2 pl-4 z-10 shrink-0">
                        ...
                        {isEditor && !isFormDisabled && (
                          <Button type="button" variant="outline" size="sm" onClick={(e) => { e.stopPropagation(); openDialog('reappropriation', createDefaultReappropriationDetail()); }} disabled={isSupervisor || isInvestigator || isViewer}><PlusCircle className="mr-2 h-4 w-4" />Add</Button>
                        )}
                      </div>
                ...
                <CardHeader className="flex flex-row justify-between items-start">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">Test wells, logging records, and pumping test parameters</p>
                    </div>
                  </div>
              ...
              <Card className="border-l-4 border-l-rose-600 dark:border-l-rose-500 shadow-xs">
                <CardHeader className="flex flex-row justify-between items-start">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <CardTitle className="text-xl font-bold tracking-tight text-foreground">{paymentDetailsSectionNumber}. Payment Details</CardTitle>
