...
import { FormSectionStepper, type FormSection } from "@/components/shared/FormSectionStepper";
...
const createDefaultReappropriationDetail = (): ReappropriationDetailFormData => ({ type: "Outward", refFileNo: "", siteName: "", asGiven: undefined, expenditure: null, amount: undefined, date: "", remarks: "", pageType: "Logging & Pumping Test", fileDetails: "" });
...
export default function LoggingPumpingTestDataEntryFormComponent({ fileNoToEdit, initialData, allStaffMembers, userRole, workTypeContext, returnPath, pageToReturnTo, isFormDisabled = false, allLsgConstituencyMaps }: DataEntryFormProps) {
  ...
  const paymentDetailsSectionNumber = 5;
  const finalDetailsSectionNumber = 6;
  ...
  const formSections: FormSection[] = [
    { id: 'section-application', label: '1. Application', count: watch('applicantName') ? 1 : 0 },
    { id: 'section-remittance', label: isDeferredFunding ? '2. Admin Sanction' : '2. Remittance', count: (remittanceFields || []).length },
    { id: 'section-reappropriation', label: '3. Re-appropriation', count: (sortedCombinedReappropriations || []).length },
    { id: 'section-sites', label: '4. Sites', count: (siteFields || []).length },
    { id: 'section-payments', label: `${paymentDetailsSectionNumber}. Payments`, count: (paymentFields || []).length },
    { id: 'section-final', label: `${finalDetailsSectionNumber}. Final & Summary` },
  ];
    ...
    <FormProvider {...form}>
      <div>
        <form onSubmit={handleSubmit(onSubmit, onInvalid)} className="space-y-6">
            {/* Top Sticky Stepper & Budget Bar */}
            <div className="sticky top-14 z-30 space-y-2 bg-background/95 backdrop-blur-md pt-1 pb-2 border-b border-border/40 shadow-xs">
                <FormSectionStepper sections={formSections} />
                <FormBudgetSummaryBar
                    totalRemittance={totalRemittanceWatched || 0}
                    totalReappropriationCredit={totalReappropriationCreditWatched || 0}
                    totalPayment={totalPaymentWatched || 0}
                    totalReappropriationDebit={totalReappropriationWatched || 0}
                    overallBalance={watch('overallBalance') || 0}
                    isPlanFund={isDeferredFunding}
                />
            </div>
            ...
            <Card id="section-payments"><CardHeader className="flex flex-row justify-between items-start"><div><CardTitle className="text-xl">{paymentDetailsSectionNumber}. Payment Details</CardTitle></div>{isEditor && !isFormDisabled && <Button type="button" onClick={() => openDialog('payment', createDefaultPaymentDetail())} disabled={isSupervisor || isInvestigator || isViewer}><PlusCircle className="h-4 w-4 mr-2" />Add</Button>}</CardHeader><CardContent><div className="w-full overflow-x-hidden"><Table><TableHeader><TableRow><TableHead>Date</TableHead><TableHead>Account</TableHead><TableHead className="text-right w-24 sm:w-28 leading-tight">Total Payment<br /><span className="text-xs font-normal text-muted-foreground">(₹)</span></TableHead><TableHead className="min-w-[180px]">Name of Site</TableHead><TableHead>Remarks</TableHead>{isEditor && !isFormDisabled && <TableHead>Actions</TableHead>}</TableRow></TableHeader><TableBody>{sortedPaymentFields.length > 0 ? sortedPaymentFields.map((item) => (
