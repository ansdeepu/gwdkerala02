...
import { Loader2, Trash2, PlusCircle, X, Save, Clock, Eye, ArrowUpDown, Copy, Info, ChevronLeft, ChevronRight, Edit, Move, Printer, FileText, Layers, CheckCircle2, CreditCard, ArrowLeftRight, MapPin, Receipt, FileSpreadsheet } from "lucide-react";
import { calculateSiteExpenditure } from "@/components/shared/DataEntryForm";
import { FormBudgetSummaryBar } from "@/components/shared/FormBudgetSummaryBar";
import { FormSectionStepper, type FormSectionItem } from "@/components/shared/FormSectionStepper";
...
export default function InvestigationDataEntryFormComponent({ fileNoToEdit, initialData, allStaffMembers, userRole, workTypeContext, returnPath, pageToReturnTo, isFormDisabled = false, allLsgConstituencyMaps }: DataEntryFormProps) {
  ...
  const overallBalanceWatched = watch('overallBalance') ?? 0;

  const stepperSections: FormSectionItem[] = [
    { id: 'section-application', label: 'Application', icon: <FileSpreadsheet className="h-3.5 w-3.5" />, badge: watch('fileNo') ? String(watch('fileNo')) : undefined },
    { id: 'section-remittance', label: isDeferredFunding ? 'AS Sanction' : 'Remittance', icon: <CreditCard className="h-3.5 w-3.5" />, badge: remittanceFields.length > 0 ? remittanceFields.length : undefined },
    { id: 'section-reappropriation', label: 'Re-appropriation', icon: <ArrowLeftRight className="h-3.5 w-3.5" />, badge: sortedCombinedReappropriations.length > 0 ? sortedCombinedReappropriations.length : undefined },
    { id: 'section-sites', label: 'Sites', icon: <MapPin className="h-3.5 w-3.5" />, badge: siteFields.length > 0 ? siteFields.length : undefined },
    { id: 'section-payments', label: 'Payments', icon: <Receipt className="h-3.5 w-3.5" />, badge: sortedPaymentFields.length > 0 ? sortedPaymentFields.length : undefined },
    { id: 'section-final', label: 'Final Details', icon: <CheckCircle2 className="h-3.5 w-3.5" /> },
    { id: 'section-print', label: 'Print Reports', icon: <Printer className="h-3.5 w-3.5" /> },
  ];

  return (
    <FormProvider {...form}>
      <div className="space-y-4">
        {/* Sticky Quick-Jump Section Stepper */}
        <FormSectionStepper sections={stepperSections} />

        {/* Live Budget & Expenditure Sticky Summary Bar */}
        <FormBudgetSummaryBar
          totalRemittance={totalRemittanceWatched ?? 0}
          totalReappCredit={totalReappropriationCreditWatched ?? 0}
          totalReappDebit={totalReappropriationWatched ?? 0}
          totalExpenditure={totalPaymentWatched ?? 0}
          balance={overallBalanceWatched}
        ...
        <form onSubmit={handleSubmit(onSubmit, onInvalid)} className="space-y-6">
            ...
            <Card id="section-application"><CardHeader className="flex flex-row justify-between items-start"><div><CardTitle className="text-xl">1. Application Details</CardTitle></div>{isEditor && !isFormDisabled && <Button type="button" onClick={() => openDialog('application', getValues(), false)} disabled={isSupervisor || isInvestigator || isViewer}><Eye className="h-4 w-4 mr-2" />Edit</Button>}</CardHeader><CardContent><div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4"><DetailRow label="File No." value={watch('fileNo')} /><DetailRow label="Applicant Name &amp; Address" value={watch('applicantName')} /><DetailRow label="Applicant Name &amp; Address (Malayalam)" value={watch('applicantNameMl')} /><DetailRow label="Phone No." value={watch('phoneNo')} /><DetailRow label="Secondary Mobile No." value={watch('secondaryMobileNo')} /><DetailRow label="Email ID" value={watch('emailId')} /><DetailRow label="Category" value={watch('category')} /><DetailRow label="Type of Application" value={watch('applicationType') ? applicationTypeDisplayMap[watch('applicationType') as ApplicationType] : ''} /><DetailRow label="Bank Name" value={watch('bankName')} /><DetailRow label="Branch" value={watch('branch')} /><DetailRow label="Bank Account No." value={watch('bankAccountNo')} /><DetailRow label="IFSC" value={watch('ifsc')} /></div></CardContent></Card>
            ...
            <Card id="section-final">
                <CardHeader>
                    <CardTitle className="text-xl">6. Final Details</CardTitle>
                </CardHeader>
                                            ...
                                            return (
                                                <TableRow key={site.id || idx} className="text-xs hover:bg-muted/30">
                                                    ...
                                                    <TableCell className="py-2 text-muted-foreground">
                                                        {site.purpose || site.arsTypeOfScheme || 'GW Investigation'}
                                                    </TableCell>
